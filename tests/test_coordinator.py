...
async def test_coordinator_sms_tracking_first_run(mock_hass, mock_config_entry):
    """Test that the first SMS fetch only sets the last_sms_index."""
    mock_api = MagicMock()
    coordinator = HuaweiRouter5GDataUpdateCoordinator(
        mock_hass, mock_config_entry, mock_api
    )
    ...
    data = {
        "sms_list": {
            "Messages": {
                "Message": [
                    {"Index": "5", "Phone": "123", "Content": "A", "Date": "D1"},
                    {"Index": "10", "Phone": "456", "Content": "B", "Date": "D2"},
                ]
            }
        }
    }
    ...
    assert coordinator.last_sms_index == 10
...
async def test_coordinator_sms_event_firing(mock_hass, mock_config_entry):
    ...
    coordinator.last_sms_index = 10

    data = {
        "sms_list": {
            "Messages": {
                "Message": [
                    {"Index": "10", "Phone": "123", "Content": "Old", "Date": "D0"},
                    {"Index": "11", "Phone": "456", "Content": "New1", "Date": "D1"},
                    {"Index": "12", "Phone": "789", "Content": "New2", "Date": "D2"},
                ]
            }
        }
    }
    ...
    assert coordinator.last_sms_index == 12
