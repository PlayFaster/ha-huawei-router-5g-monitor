...
where the pattern was implemented and validated first — see that project's
``.notes/device_registry/device_model_2026_08.md`` for the full rationale. The
third UniFi shim (``owning_entry_ids``, wrapping ``DeviceEntry.config_entries``)
is deliberately **not** carried over: this integration never inspects a device's
owning entries, and an unused shim would be dead code.
"""
