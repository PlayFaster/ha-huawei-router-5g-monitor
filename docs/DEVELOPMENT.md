...
### A Budget Imposed From Outside Can Only Abandon Work (v1.2.0-dev57)
...
- **The hardware check wrote no file, in either tier.** It printed results and kept them in a list, so no run could be examined afterwards and `skip()` left no trace at all — a skipped check was indistinguishable later from one nobody had written. Both tiers now file a report from a `finally`, so an aborted run still leaves what it had. Identifying values — the SIM's own number, message text — go to a second gitignored file; the shared report keeps verdicts and a redaction marker.
- **A check gated on the device's resting state is a check that does not run.** The guest-SSID toggle skipped itself whenever the WiFi radio was off, which is the normal state on the reference unit — so the one write touching an **open, unauthenticated** network was the least exercised of the set. "Cannot be observed with the radio off" is a reason to turn the radio on and put it back, which is what `set_wifi` in the same run already does.
- **Calling the API object directly cannot reproduce a concurrency defect.** Every check had one caller and no coordinator, so lock contention, debounce and entity availability were all out of reach — and the deadlock that prompted this work lived exactly there. `ha_contention` drives the integration through Home Assistant's REST API instead: change the mode, press Refresh immediately, then assert the entity is neither `unavailable` nor stale. Against the pre-fix code it would have failed.
